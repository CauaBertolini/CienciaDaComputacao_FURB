import java.util.Scanner;

public class Exe13 {
    /*
     * Dois carros (X e Y) partem em uma mesma direção. 
     * O carro X sai com velocidade constante de 60 Km/h 
     * e o carro Y sai com velocidade constante de 90 Km/h.
     * Em uma hora (60 minutos) o carro Y consegue se 
     * distanciar 30 quilômetros do carro X, ou seja, 
     * consegue se afastar um quilômetro a cada 2 minutos. 
     * Leia a distância (em Km) e calcule quanto tempo leva
     * (em minutos) para o carro Y tomar essa distância do 
     * outro carro. 
     * Análise: valores de entrada ok, enunciado demasiadamente extenso e sem
     * utilidade geral, apenas os dados de distanciamento por minutos
     * já seriam suficientes.
     * Entradas: distancia
     * Saídas: tempo
     * Processo: tempo =  distancia * 2
     */
    public static void main(String[] args) throws Exception {
        Scanner scanner = new Scanner(System.in);
        System.out.print("Distância: ");
        int distancia = scanner.nextInt();

        int tempo = distancia * 2;

        System.out.println("Levará " + tempo + " minuto(s) para se distanciar " + distancia + "km");

        scanner.close();

    }
}
